...
public class World {
	...
	*/
	...
	for (int rx = 0; rx < 3; rx++) {
		...
		for (int x = -9; x < 10; x++) {
		    for (int y = -9; y < 10; y++) {
			regions[rx][ry].tiles[x+10][y+10] = new Tile(client, (x * 48)
	...
	for (int i = 0; i < 20; i++) {
	    for (int j = 0; j < 20; j++) {
		x[0] = regions[0][0].tiles[i][j].face.getPoints()[0].getX();
		y[0] = regions[0][0].tiles[i][j].face.getPoints()[0].getY();
		z[0] = regions[0][0].tiles[i][j].face.getPoints()[0].getZ();
		x[1] = regions[0][0].tiles[i][j].face.getPoints()[1].getX();
		y[1] = regions[0][0].tiles[i][j].face.getPoints()[1].getY();
		z[1] = regions[0][0].tiles[i][j].face.getPoints()[1].getZ();
		x[2] = regions[0][0].tiles[i][j].face.getPoints()[2].getX();
		y[2] = regions[0][0].tiles[i][j].face.getPoints()[2].getY();
		z[2] = regions[0][0].tiles[i][j].face.getPoints()[2].getZ();
		x[3] = regions[0][0].tiles[i][j].face.getPoints()[3].getX();
		y[3] = regions[0][0].tiles[i][j].face.getPoints()[3].getY();
		z[3] = regions[0][0].tiles[i][j].face.getPoints()[3].getZ();
