...
import com.ctp.packets.DataType;
import com.ctp.packets.PacketData;
import com.ctp.packets.Request;
import com.ctp.packets.SingleDataPacket;
		...
		if (userFile.exists() && !userFile.isDirectory()) {
			...
			try {
				...
				if (profile.username.equalsIgnoreCase(username)) {
					if (profile.verifyPassword(password)) {
						if (!loggedIn(profile.ID)) {
							try {
								...
								SingleDataPacket packet = (SingleDataPacket) new SingleDataPacket(new PacketData(DataType.STRING, false)).setRequest(Request.ERROR_LOGIN);
								packet.getData().setObject(new String(
							...
							users.add(profile);
							SingleDataPacket packet = new SingleDataPacket(new PacketData(DataType.STRING, false));
							packet.setRequest(Request.SUCCESSFUL_LOGIN);
							packet.getData().setObject(new String("Welcome to " + server
									+ "!")).setDataType(DataType.STRING);
							server.getServerIO().sendPacket(subServerIndex,
									profile.ID, packet);
							server.getServerIO().print(
							...
							packet.getData().setObject(new String("Error: That profile is already logged in.")).setDataType(DataType.STRING);
							server.getServerIO().sendPacket(subServerIndex,
									userID, packet);
							return false;
						}
						...
						SingleDataPacket packet = new SingleDataPacket(new PacketData(DataType.STRING, false));
						packet.setRequest(Request.ERROR_LOGIN);
						packet.getData().setObject(new String("Incorrect password.")).setDataType(DataType.STRING);
					...
					SingleDataPacket packet = new SingleDataPacket(new PacketData(DataType.STRING, false));
					packet.setRequest(Request.ERROR_LOGIN);
					packet.getData().setObject(new String("There was an error recognizing your account. \n If problem persists, contact support.")).setDataType(DataType.STRING);
			...
			try {
				...
				SingleDataPacket packet = new SingleDataPacket(new PacketData(DataType.STRING, false));
				packet.setRequest(Request.ERROR_LOGIN);
				packet.getData().setObject(new String("Error occured while loggin in. Try again later.")).setDataType(DataType.STRING);
				server.getServerIO().sendPacket(subServerIndex, userID, packet);
				return false;
			}
			SingleDataPacket packet = new SingleDataPacket(new PacketData(DataType.STRING, false));
			packet.setRequest(Request.SUCCESSFUL_LOGIN);
			packet.getData().setObject(new String("Welcome to "+server)).setDataType(DataType.STRING);
			try {
				server.getServerIO().sendPacket(subServerIndex,
		...
		if (userFile.exists() && !userFile.isDirectory()) {
			...
			try {
				...
				if (profile.username.equalsIgnoreCase(username)) {
					if (profile.verifyPassword(password)) {
						if (!loggedIn(profile.ID)) {
							try {
								...
								SingleDataPacket packet = new SingleDataPacket(new PacketData(DataType.STRING, false));
								packet.setRequest(Request.ERROR_LOGIN);
								packet.getData().setObject(new String("Error occured while logging in. Try again later.")).setDataType(DataType.STRING);
								server.getServerIO().sendPacket(subServerIndex,
										userID, packet);
								return false;
							}
					...
					SingleDataPacket packet = new SingleDataPacket(new PacketData(DataType.STRING, false));
					packet.setRequest(Request.ERROR_LOGIN);
					packet.getData().setObject(new String("There was a problem recognizing your profile. \n If problem persists, please contact support.")).setDataType(DataType.STRING);
					server.getServerIO().sendPacket(subServerIndex, profile.ID,
							packet);
					return false;
				}
				...
				if (!backup)
					...
					SingleDataPacket packet = new SingleDataPacket(new PacketData(DataType.STRING, false));
					packet.setRequest(Request.ERROR_LOGIN);
					packet.getData().setObject(new String("An exception occurred. Retry a few times. \n If that doesn't work, contact staff.")).setDataType(DataType.STRING);
					server.getServerIO().sendPacket(subServerIndex, userID,
							packet);
					server.getServerIO().printErr(
