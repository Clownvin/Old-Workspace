...
import com.limeapi.lang.CString;
import com.limeapi.lang.CorruptDataException;
import com.limeapi.util.BinaryOperations;
import com.limeapi.util.DataType;
import com.limeapi.util.Utilities;
...
public class Packet {
	...
	Packet packet = new Packet(Protocall.NONE, Request.NONE,
		new ArrayList<PacketData>()).addData(data);
	return packet;
    }

    public static Packet buildPacket(Protocall p) {
	Packet packet = new Packet(p, Request.NONE, null);
	return packet;
    }

    public static Packet buildPacket(Protocall p, PacketData... data) {
	Packet packet = new Packet(p, Request.NONE, null).addData(data);
	return packet;
    }
	...
	Protocall p = Protocall.getProtocall(bytes[0][0]);
	...
	byte[] pr = new byte[2];
	pr[0] = packet.protocall.byteTag;
	pr[1] = packet.request.byteTag;
	bytes[0] = pr;
    ...
    private Packet(Protocall protocall, Request request,
