...
public final class GraphicsHandler extends Component {
    ...
    private int getEuclideanDistance(int x1, int y1, int z1, int x2, int y2, int z2) {
	return (int) (Math.pow(x2 - x1, 2) + Math.pow(y2 - y1, 2) + Math.pow(z2 - z1,  2));
    }
	...
	public int compare(Face arg0, Face arg1) {
	    if (arg0 == null || arg1 == null)
		return 0;
	    return getEuclideanDistance(arg1.averageX, arg1.averageY, arg1.averageZ, client.x, client.y, viewAngle.z) + getEuclideanDistance(arg0.averageX, arg0.averageY, arg0.averageZ, client.x, client.y, viewAngle.z);
	}
    };
	...
	int camZ = (int) ((screenPosition.z / 2) * Math.sin(phi)); // NOTE: Good to go! :D
	int camX = (int) ((int) ((screenPosition.z / 2) * Math.cos(phi)) * 1.15); // NOTE: This is the radius of the cam "Track". It is off, but only by about 200ish, or 15% (Guess)
	// MORE TRIG! :D
