...
public final class Console extends javax.swing.JFrame {
	...
	private void processCommand(String command) {
		...
		if (command.replace(" ", "").toLowerCase().equalsIgnoreCase("threads")) {
			ThreadPool.listThreads();
		}
		if (command.replace(" ", "").toLowerCase().equalsIgnoreCase("modules")) {
			ModuleManager.listModules();
		}
		if (command.replace(" ", "").toLowerCase().equalsIgnoreCase("subservers")) {
		...
		if (command.replace(" ", "").toLowerCase()
				.equalsIgnoreCase("threadcount")) {
			ServerIO.print("[ThreadPool] Current ThreadPool size: "
		...
		if (command.replace(" ", "").toLowerCase().startsWith("color")) {
			...
			consoleArea.setBackground(new Color((255-r)/10, (255-g)/10, (255-b)/10));
			commandLine.setForeground(new Color(r, g, b));
			commandLine.setBackground(new Color((255-r)/10, (255-g)/10, (255-b)/10));
		}
	}
	//TODO Document
	/**
